...
//
// ArrayHandle:
//   Like Handle, but for multiple objects of the same type.

namespace vespalib::tdl {

constexpr size_t align = 8;

constexpr size_t do_align(size_t value) {
...
public:
    ...
    template <typename T> const T& resolve(Handle h) const {
        static constexpr size_t I = type_id<T, MyDomain>();
        assert(h.type() == I);
        auto array = all_of<T>();
    ...
    template <typename T> T& resolve(Handle h) {
...
public:
    void operator()(MyData *ptr) noexcept {
        auto destruct_array = [&]<typename T>() {
            for (auto &obj : std::views::reverse(ptr->template all_of<T>())) {
                obj.~T();
            }
        };
...
public:
    ...
    DataUP<MyDomain> create_data() const {
        using MyData = Data<MyDomain>;
        auto calc_aligned_size = [&]<size_t... Is>(std::index_sequence<Is...>)->size_t {
            return (do_align(counts[Is] * sizeof(Ts)) + ...);
        };
        size_t need_size = sizeof(MyData) + calc_aligned_size(typename MyDomain::index_sequence{});
        char *mem = static_cast<char *>(::operator new(need_size));
        auto *obj = new (mem) MyData(need_size);
        size_t offset = sizeof(MyData);
        [&]<size_t... Is>(std::index_sequence<Is...>) {
            auto construct_array = [&]<typename T, size_t I>() {
                obj->_header[I] = {uint32_t(offset), counts[I]};
                T *arr = reinterpret_cast<T*>(mem + offset);
                for (uint32_t j = 0; j < counts[I]; ++j) {
                    new (arr + j) T();
                }
                offset += do_align(counts[I] * sizeof(T));
            };
            (construct_array.template operator()<Ts, Is>(), ...);
        }(typename MyDomain::index_sequence{});
        assert(offset == need_size);
        return DataUP<MyDomain>(obj);
    }
};
